...
const KNOWLEDGE_BASE = `
  ...
  HƯỚNG DẪN ĐẶT LỊCH:
  - Bạn cần thu thập: Tên bệnh nhân, Số điện thoại, Triệu chứng/Lý do khám, Thời gian mong muốn.
  - YÊU CẦU ĐỊNH DANH:
  ...
  const extractBookingData = async (history: Message[]): Promise<any> => {
    try {
      ...
      const response = await genAI.models.generateContent({
        ...
        contents: [
          { role: "user", parts: [{ text: `Dựa trên lịch sử trò chuyện, hãy trích xuất thông tin đặt lịch khám bệnh dưới dạng JSON với các trường: patientName, phone, department, appointmentTime (định dạng YYYY-MM-DDTHH:mm), patientId (Mã số bệnh nhân nếu là BN cũ), citizenId (Số CCCD nếu là BN mới). Nếu thiếu thông tin nào hãy để null. Chỉ trả về JSON.\n\nLịch sử:\n${JSON.stringify(history)}` }] }
        ],
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden relative">
      ...
      <div className="bg-emerald-600 p-4 flex items-center gap-3 text-white shadow-md z-10">
        ...
        <div>
          <h2 className="font-bold text-lg leading-tight">Tổng đài AI Liên Chiểu</h2>
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 bg-emerald-300 rounded-full animate-pulse"></span>
            <p className="text-xs text-emerald-50 font-medium">Sẵn sàng hỗ trợ 24/7</p>
          </div>
        </div>
      </div>
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center space-y-6 max-w-md mx-auto">
            <div className="bg-white p-6 rounded-3xl shadow-xl shadow-emerald-100/50 border border-emerald-50">
              ...
              <h3 className="text-xl font-bold text-slate-900 mb-2">Chào mừng bạn!</h3>
            ...
            <div className="grid grid-cols-2 gap-3 w-full">
              {['Đặt lịch khám', 'Đau dạ dày', 'Khám thai', 'Sốt cao'].map((q) => (
                <button
                  ...
                  className="px-4 py-3 bg-white hover:bg-emerald-50 border border-slate-200 hover:border-emerald-200 rounded-2xl text-xs font-semibold text-slate-700 transition-all shadow-sm text-left flex items-center gap-2"
                >
      ...
      <div className="p-4 bg-white border-t border-slate-100 shrink-0">
        <div className="max-w-4xl mx-auto flex items-center gap-3 bg-slate-50 p-2 rounded-2xl border border-slate-200 focus-within:border-emerald-300 focus-within:ring-4 focus-within:ring-emerald-50 transition-all">
          ...
          <input
            ...
            placeholder="Nhập tin nhắn hoặc triệu chứng của bạn..."
            className="flex-1 bg-transparent border-none focus:ring-0 text-sm py-2 px-2 text-slate-800 placeholder:text-slate-400 font-medium"
          />
          <button
            onClick={() => handleSend()}
            disabled={loading || !input.trim()}
            className="p-3 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 text-white rounded-xl transition-all shadow-lg shadow-emerald-100 disabled:shadow-none shrink-0"
          >
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="bg-emerald-600 p-6 text-white relative">
              ...
              <h3 className="text-xl font-bold mb-1">Xác nhận lịch hẹn</h3>
              <p className="text-emerald-50 text-sm opacity-90">Vui lòng kiểm tra lại thông tin bên dưới</p>
            </div>
            ...
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-1 gap-4">
                <div className="flex items-center gap-4 p-3 rounded-2xl bg-slate-50 border border-slate-100">
                  ...
                  <div className="flex-1">
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Bệnh nhân</p>
                    <input 
                ...
                <div className="flex items-center gap-4 p-3 rounded-2xl bg-slate-50 border border-slate-100">
                  ...
                  <div className="flex-1">
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Mã số BN / CCCD</p>
                    <input 
                      className="w-full bg-transparent text-sm font-bold text-slate-900 focus:outline-none"
                      ...
                      onChange={e => {
                        ...
                        // Heuristic: if length is 12 it's likely CCCD, otherwise patientId
                        if (val.length === 12) {
                ...
                <div className="flex items-center gap-4 p-3 rounded-2xl bg-slate-50 border border-slate-100">
                  ...
                  <div className="flex-1">
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Khoa khám</p>
                    <select 
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 px-4 py-3 border border-slate-200 text-slate-600 font-bold rounded-2xl hover:bg-slate-50 transition-colors text-sm"
                >
                  Hủy bỏ
                </button>
                <button
                  ...
                  className="flex-1 px-4 py-3 bg-emerald-600 text-white font-bold rounded-2xl hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100 flex items-center justify-center gap-2 text-sm disabled:bg-slate-300 disabled:shadow-none"
                >
